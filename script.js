    ...
    if (results.length > 0) {
        results.forEach(result => {
            ...
            resultElement.innerHTML = `
                <h2><a href="content/${result.file}">${result.file.replace('.html', '').replace(/-/g, ' ')}</a></h2>
                <p>Matching content: ...${extractSnippet(result.content, query)}...</p>
            `;
...
function extractSnippet(content, query) {
    ...
    return content.substring(start, end).replace(new RegExp(query, 'gi'), match => `<mark>${match}</mark>`);
}
